...
public class EggsController {
  ...
  /**
   * Returns a 200 response at the '/wallet/add' path. Check if service is running
   *
   * @return response entity
   */
  @PostMapping(value = Endpoints.EGG_ADD_ID)
  @PreAuthorize("hasRole('ROLE_ADMIN') or hasRole('ROLE_CLIENT')")
  public ResponseEntity<BaseApiResponse> postBoilEggs(
      @PathVariable("id") String id, @RequestBody @Valid CookRequest payload) {
    try {
      log.debug("data from payload {}", payload.getUsername());
      return new ResponseEntity<>(
          new BaseApiResponse("OK", "Not Implemented", payload), HttpStatus.OK);
