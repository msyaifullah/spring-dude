...
public class FtpClient {
    ...
    public void initFtpClient() {
        ...
        // cancel server to obtain Ip address and host their own submission match, otherwise it will report an exception when inconsistent.
        ...
        try {
            ...
            log.warn("[initFtpClient]: Use account:" + username + "Password:" + password + "sign in the ftp server:" + host + ":" + port);
            log.warn("[initFtpClient]: successful login server, passive mode the host:" + ftpClient.getPassiveHost() + ":" + ftpClient.getPassivePort());
            log.warn("[initFtpClient]: logging into the server, the active mode of the host:" + ftpClient.getRemoteAddress() + ":" + ftpClient.getRemotePort());
            log.warn("[initFtpClient]: logging into the server localhost:" + ftpClient.getLocalAddress() + ":" + ftpClient.getLocalPort());
            log.warn("[initFtpClient]: logging into the server, return code:" + ftpClient.getReplyCode() + ", displays the status of" + ftpClient.getStatus());
     ...
     * @Param fileName to ftp upload file name
    ...
    public boolean uploadFile(String pathname, String fileName, InputStream inputStream) {
        ...
        try {
            ...
            initFtpClient();
            ftpClient.setFileType(FTP.BINARY_FILE_TYPE); // set the transmission mode to binary file transfer type
            ftpClient.makeDirectory(pathname); // Set directory
            ftpClient.changeWorkingDirectory(pathname); // set the working path

            ftpClient.enterLocalPassiveMode(); // set the passive mode (FTP client in the docker container required passive mode)
